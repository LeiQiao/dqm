...
def request_web():
    rsp = requests.get('http://dqm.ffsky.cn/all.htm')
...
def parse_monster(mtr, kind_name):
    ...
    monster.image_name = re.search('<a href=\"([\\w\\W]*?)\"', tds[1]).group(1)
    monster.name = monster.jp_name = re.search('<font[\\w\\W]*?>([\\w\\W]*?)</font>', tds[1]).group(1)
    tds[2] = tds[2].replace('<p>', '').replace('</p>', '')
    ...
    for k in kinds:
        for m in k.monsters:
            ...
            if os.path.exists(file_name):
                ...
                continue
            rsp = requests.get('http://dqm.ffsky.cn/'+m.image_name)
            if rsp.status_code != 200:
                m.image_name = None
                continue
